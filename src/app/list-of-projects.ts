import { Project } from './project';

export const PROJECTS: Project[] = [

  {
    id: "janalytics",
    title: "JAnalytics",
    category: "web data fav",
    status: "coming-soon",
    duration: "December 2017 - present",
    type: "client project",
    role: "Lead Developer, Data Analyst",
    // role_description: "designed the pipeline to ___",
    description: "developing and maintaining data visualization dashboard",
    overview: "An analytics dashboard designed to help visualize data in multi-dimensions. A tool to monitor and compare the performance patent applications and law firms",
    tools: ["MongoDB", "D3", "Flask"],
    color: "navy",
  },

  {
    id: "chess-store",
    title: "A&M Chess Store",
    category: "web",
    status: "done",
    duration: "Spring 2017",
    type: "Application Design and Development",
    //type: "Course Project | Application Design & Development",
    //type: "Course | Application Design & Development",
    role: "Full-Stack Developer",
    // role_description: "",
    description: "eCommerce web app using ruby on rails",
    overview: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam interdum at lacus vel posuere. Proin non neque imperdiet, venenatis lacus ut, ultrices metus.",
    tools: ["Ruby on Rails", "HTML5", "CSS3"],
    color: "red",
  },

  {
    id: "airbnb-data",
    title: "Case Study: Airbnb",
    category: "data",
    status: "done",
    duration: "March 2017",
    type: "Class Project | Practical Data Science",
    role: "Data Analyst",
    description: "wrangled data using Pandas to understand trends of Airbnb listings",
    overview: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam interdum at lacus vel posuere. Proin non neque imperdiet, venenatis lacus ut, ultrices metus.",
    tools: ["Pandas", "BeautifulSoup", "Tableau"],
    color: "light-blue",
  },

  {
    id: "asa-booth",
    title: "ASA Booth",
    category: "misc",
    status: "done",
    duration: "Sept 2016 - April 2017",
    type: "Asian Students Assocation",
    role: "Booth Chair",
    description: "designed and built a 2 story building",
    overview: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam interdum at lacus vel posuere. Proin non neque imperdiet, venenatis lacus ut, ultrices metus.",
    tools: [],
    color: "light-blue",
  },

  {
    id: "portfolio",
    title: "My Portfolio",
    category: "web",
    status: "done",
    duration: "May 2018",
    type: "Personal",
    role: "Developer + Designer",
    description: "personal site to display my works",
    overview: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam interdum at lacus vel posuere. Proin non neque imperdiet, venenatis lacus ut, ultrices metus.",
    tools: ["AngularJS"],
    color: "navy",
  },

  // {
  //   id: 4,
  //   title: "Big Historical Data",
  //   category: "data",
  //   status: "done",
  //   duration: "Spring 2017",
  //   role: "Data Analyst, Full-Stack Developer",
  //   description: "digital humanities - analyzed corpus historical data",
  //   tools: ["Pandas", "NLTK", "Ruby on Rails"],
  //   color: "navy",
  // },


  // {
  //   id: 5,
  //   title: "Thin Ice",
  //   category: "misc",
  //   status: "done",
  //   duration: "Dec 2015",
  //   role: "Game Developer",
  //   description: "developed a maze game using machine learning technqiues to randomize levels",
  //   tools: ["Python", "PyGame"],
  //   color: "red"
  // },

  // // {
  // //   id: 6,
  // //   title: "Predicting Flight Delays",
  // //   category: "data",
  // //   status: "done",
  // //   duration: "May 2017",
  // //   role: "Data Scientist",
  // //   description: "created machines learning tests",
  // //   tools: ["Pandas"],
  // //   color: "red"
  // // },

  // {
  //   id: 5,
  //   title: "Prantls",
  //   category: "web",
  //   status: "done",
  //   duration: "March 2016",
  //   role: "Front-End Developer",
  //   description: "my first web development project",
  //   tools: ["HTML5", "CSS3"],
  //   color: "light-blue"
  // },

  // {
  //   id: 7,
  //   title: "sjark consulting",
  //   category: "web",
  //   status: "done",
  //   duration: "April 2016",
  //   role: "Product Manager, Front-End Developer",
  //   description: "technical case study with a dashboard",
  //   tools: ["HTML5", "CSS3"],
  //   color: "navy"
  // },

  // {
  //   id: ,
  //   title: "",
  //   duration: "",
  //   role: "",
  //   description: "",
  //   tools: []
  // },

];